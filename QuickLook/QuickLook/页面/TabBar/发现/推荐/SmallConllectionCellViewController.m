//
//  SmallConllectionCellViewController.m
//  QuicklyLook
//
...
@interface SmallConllectionCellViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation SmallConllectionCellViewController
...
- (void)viewDidLoad
{
    ...
    _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width - 20, 300) collectionViewLayout:flowlayout];
    _collectionView.delegate = self;
    _collectionView.dataSource = self;
    
    [_collectionView registerNib:[UINib nibWithNibName:@"SmallCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"cell"];
...
-(void)netWorking
{
    ...
    [self.manager GET:url parameters:nil progress:^(NSProgress * _Nonnull downloadProgress)
     ...
     } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
     {
         ...
         NSDictionary * dic = arrr[1];
         
         NSArray * arr = [PaiHangModel mj_objectArrayWithKeyValuesArray:dic[@"topics"]];
...
{
    return MIN(_muArray.count, 12);
}

-(CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    return CGSizeMake(self.view.frame.size.width - 100, 100);
}
...
{
    SmallCollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
